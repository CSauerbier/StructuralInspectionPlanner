...
{
  ros::init(argc, argv, "solarPlantRequester");
  ...
  srv.request.spaceSize.push_back(100.0);
  srv.request.spaceSize.push_back(100.0);
  srv.request.spaceSize.push_back(3.0);
  srv.request.spaceCenter.push_back(10.0);
  srv.request.spaceCenter.push_back(90.0);
  srv.request.spaceCenter.push_back(5.0);
  geometry_msgs::Pose reqPose;

  /* starting pose */
  reqPose.position.x = -35.0;
  reqPose.position.y = 90.0;
  reqPose.position.z = 5.0;
  ...
  srv.request.minDist = 3.0;
  srv.request.maxDist = 5.0;
  srv.request.numIterations = 20;

  /* read STL file and publish to rviz */
  std::vector<nav_msgs::Path> * mesh = readSTLfile(ros::package::getPath("request")+"/meshes/solarPlant.stl");
